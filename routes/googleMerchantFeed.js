...
router.get('/api/merchant-feed.xml', async (req, res) => {
  try {
    ...
    products.forEach((product) => {
      ...
      // 🔥 Pro Feature: XML CDATA Sanitization to prevent malformed XML breaks
      const safeTitle = (product.title || '').replace(/]]>/g, ']]]]><![CDATA[>');
      const safeDesc = (product.description || product.title || '').replace(/]]>/g, ']]]]><![CDATA[>');
      const safeBrand = (brand).replace(/]]>/g, ']]]]><![CDATA[>');
      const safeCategory = (product.category || 'General').replace(/]]>/g, ']]]]><![CDATA[>');

      xml += `<item>`;
      xml += `<g:id>${productId}</g:id>`;
      xml += `<g:title><![CDATA[${safeTitle}]]></g:title>`;
      xml += `<g:description><![CDATA[${safeDesc}]]></g:description>`;
      ...
      xml += `<g:brand><![CDATA[${safeBrand}]]></g:brand>`;
      xml += `<g:product_type><![CDATA[${safeCategory}]]></g:product_type>`;
      xml += `</item>`;
    });
